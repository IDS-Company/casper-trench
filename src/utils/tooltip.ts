...
export const externalEarningTooltipHandler = (context) => {
	// Tooltip Element
	const { chart, tooltip } = context;
	const tooltipEl = getOrCreateTooltip(chart);
	...
	if (tooltip.body) {
		...
		titleLines.forEach((title, i) => {
			...
			th.style.textAlign = 'left';
			th.style.width = '100%';
			const text = document.createTextNode(`${date.toLocaleString('en-us')}`);
		...
		bodyLines.forEach((body, i) => {
			const colors = tooltip.labelColors[i];
			const bodyData = body[0].toString().split(':');
			...
			const textLeft = document.createTextNode(bodyData[0]);
			const textRight = document.createTextNode(bodyData[1]);
